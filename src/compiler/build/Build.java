...
public class Build {
	...
	public void build(LinkedList<ImcChunk> chunks) {
		for (ImcChunk chunk : chunks) {
			...
			final List<AsmInstr> prologue = new LinkedList<>();
			final List<AsmInstr> epilogue = new LinkedList<>();
			...
			if (!codeChunk.registers.get(codeChunk.frame.RV).equals("$0")) {
				epilogue.add(new AsmOPER("SET", "$0,`s0", null, new LinkedList<>(Arrays.asList(codeChunk.frame.RV))));
			}
		...
		if (functions.contains("_get_int")) {
			final ImcCodeChunk get_int = new ImcCodeChunk(null, null);
			get_int.asmcode = new LinkedList<>();
		...
		if (functions.contains("_put_int")) {
			final ImcCodeChunk put_int = new ImcCodeChunk(null, null);
			put_int.asmcode = new LinkedList<>();
		...
		if (functions.contains("_put_nl")) {
			final ImcCodeChunk put_nl = new ImcCodeChunk(null, null);
			put_nl.asmcode = new LinkedList<>();
		...
		if (functions.contains("_put_int") || functions.contains("_get_int")) {
		...
		if (functions.contains("_get_str")) {
			final ImcCodeChunk get_str = new ImcCodeChunk(null, null);
			get_str.asmcode = new LinkedList<>();
		...
		if (functions.contains("_put_str")) {
			final ImcCodeChunk put_str = new ImcCodeChunk(null, null);
			put_str.asmcode = new LinkedList<>();
		...
		if (functions.contains("_get_char_at")) {
			final ImcCodeChunk get_char_at = new ImcCodeChunk(null, null);
			get_char_at.asmcode = new LinkedList<>();
		...
		if (functions.contains("_put_char_at")) {
			final ImcCodeChunk put_char_at = new ImcCodeChunk(null, null);
			put_char_at.asmcode = new LinkedList<>();
		...
		final ImcCodeChunk main = new ImcCodeChunk(null, null);
			...
			if (chunk instanceof ImcDataChunk) {
				...
				for (AsmInstr instr : ((ImcCodeChunk) chunk).asmcode) {
					if (instr.labels.size() > 0) {
						final int length = instr.labels.getFirst().name().length();
