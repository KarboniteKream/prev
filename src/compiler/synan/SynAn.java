...
public class SynAn {
	...
	private void readNext(boolean allowEOF) {
		...
		if (currSymbol.token == Token.EOF && !allowEOF) {
	...
	public AbsTree parse() {
		...
		final AbsTree tree = parse_definitions();
		readNext(true);

		if (currSymbol.token != Token.EOF) {
			Report.error(currSymbol.position, "Unparseable symbol.");
		}
	...
	private AbsDefs parse_definitions() {
		...
		final Position position = currSymbol.position;
		final Vector<AbsDef> defs = new Vector<>();
	...
	private AbsDef parse_definition() {
		...
		if (currSymbol.token == Token.KW_TYP) {
			...
			final String name = prevSymbol.lexeme;
			check(Token.COLON);
			final AbsType type = parse_type();
			...
			final String name = prevSymbol.lexeme;
			check(Token.LPARENT);
			final Vector<AbsPar> parameters = parse_parameters();
			check(Token.RPARENT);
			check(Token.COLON);
			final AbsType type = parse_type();
			check(Token.ASSIGN);
			final AbsExpr expression = parse_expression();
	...
	private AbsType parse_type() {
		...
		switch (currSymbol.token) {
			...
			case Token.KW_ARR:
				...
				final String size = prevSymbol.lexeme;
				check(Token.RBRACKET);
				final AbsType arrType = parse_type();
			...
			case Token.KW_REC:
				...
				prepareNext();
				check(Token.LBRACE);
				final Vector<AbsComp> components = parse_components();
				check(Token.RBRACE);
			...
			case Token.PTR:
				dump("type -> ^ type");
				prepareNext();
				final AbsType ptrType = parse_type();
	...
	private Vector<AbsComp> parse_components() {
		...
		final Vector<AbsComp> components = new Vector<>();
	...
	private AbsComp parse_component() {
		...
		check(Token.IDENTIFIER);
		final Position position = prevSymbol.position;
		final String name = prevSymbol.lexeme;
		check(Token.COLON);
		final AbsType type = parse_type();
	...
	private Vector<AbsPar> parse_parameters() {
		...
		final Vector<AbsPar> parameters = new Vector<>();
	...
	private Vector<AbsExpr> parse_expressions() {
		...
		final Vector<AbsExpr> expressions = new Vector<>();
	...
	private AbsExpr parse_expression() {
		...
		final AbsDefs definitions = parse_expression_();
	...
	private AbsDefs parse_expression_() {
		...
		final AbsDefs definitions = parse_definitions();
		check(Token.RBRACE);
	...
	private AbsExpr parse_logical_ior_expression_(AbsExpr left) {
		...
		if (currSymbol.token == Token.IOR) {
			dump("logical_ior_expression' -> | logical_and_expression logical_ior_expression'");
			prepareNext();
			final AbsExpr right = parse_logical_and_expression();
	...
	private AbsExpr parse_logical_and_expression_(AbsExpr left) {
		...
		if (currSymbol.token == Token.AND) {
			dump("logical_and_expression' -> & comparative_expression logical_and_expression'");
			prepareNext();
			final AbsExpr right = parse_comparative_expression();
	...
	private AbsExpr parse_additive_expression_(AbsExpr left) {
		...
		if (currSymbol.token == Token.ADD) {
			dump("additive_expression' -> + multiplicative_expression additive_expression'");
			prepareNext();
			final AbsExpr right = parse_multiplicative_expression();
	...
	private AbsExpr parse_multiplicative_expression_(AbsExpr left) {
		...
		if (currSymbol.token == Token.MUL) {
			dump("multiplicative_expression' -> * prefix_expression multiplicative_expression'");
			prepareNext();
			final AbsExpr right = parse_prefix_expression();
	...
	private AbsExpr parse_prefix_expression() {
		...
		final AbsExpr expression;
		final Position position = currSymbol.position;

		if (currSymbol.token == Token.ADD) {
			dump("prefix_expression -> + prefix_expression");
			prepareNext();
			final AbsExpr expr = parse_prefix_expression();
	...
	private AbsExpr parse_postfix_expression_(Position position, AbsExpr left) {
		...
		if (currSymbol.token == Token.PTR) {
			...
			prepareNext();
			final AbsExpr array = parse_expression();
			check(Token.RBRACKET);
	...
	private AbsExpr parse_atom_expression() {
		...
		if (currSymbol.token == Token.LOG_CONST) {
			...
			final String name = currSymbol.lexeme;
			prepareNext();
			final Vector<AbsExpr> expressions = parse_atom_expression_identifier();
			...
			prepareNext();
			final Vector<AbsExpr> expressions = parse_expressions();
			check(Token.RPARENT);
	...
	private AbsExpr parse_atom_expression_braces() {
		...
		if (currSymbol.token == Token.KW_IF) {
			...
			prepareNext();
			final AbsExpr ifExpression = parse_expression();
			check(Token.KW_THEN);
			final AbsExpr thenExpression = parse_expression();
			final AbsExpr elseExpression = parse_atom_expression_else();
			...
			prepareNext();
			final AbsExpr condition = parse_expression();
			check(Token.COLON);
			final AbsExpr body = parse_expression();
			readNext();
			...
			prepareNext();
			final AbsExpr count = parse_expression();
			check(Token.ASSIGN);
			final AbsExpr lo = parse_expression();
			check(Token.COMMA);
			final AbsExpr hi = parse_expression();
			check(Token.COMMA);
			final AbsExpr step = parse_expression();
			check(Token.COLON);
			final AbsExpr body = parse_expression();
			readNext();
			...
			final AbsExpr left = parse_expression();
			check(Token.ASSIGN);
			final AbsExpr right = parse_expression();
			readNext();
